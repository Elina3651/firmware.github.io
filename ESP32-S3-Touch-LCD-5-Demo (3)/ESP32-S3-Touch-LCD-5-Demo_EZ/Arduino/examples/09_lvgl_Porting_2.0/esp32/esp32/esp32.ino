...
void handleCommand(const String& cmd) {
    ...
    if (strcmp(command, "START_PROCESS") == 0) {
        ...
        currentProcess.primary_uv_type1_enabled = params["primary_uv_type1_enabled"];
        currentProcess.primary_uv_type1_flickers = params["primary_uv_type1_flickers"];
        currentProcess.primary_uv_type2_enabled = params["primary_uv_type2_enabled"];
        currentProcess.primary_uv_type2_flickers = params["primary_uv_type2_flickers"];
        currentProcess.secondary_uv_exposure_sec = params["secondary_uv_exposure_sec"];
        currentProcess.secondary_uv_type1_enabled = params["secondary_uv_type1_enabled"];
        currentProcess.secondary_uv_type2_enabled = params["secondary_uv_type2_enabled"];
...
void enterState(ProcessState newState) {
    ...
    switch (currentState) {
        ...
        case SECONDARY_UV:
            ...
            if (currentProcess.secondary_uv_type1_enabled) digitalWrite(UV_380_PIN, HIGH);
            if (currentProcess.secondary_uv_type2_enabled) digitalWrite(UV_430_PIN, HIGH);
...
void state_machine_loop() {
    ...
    switch (currentState) {
        ...
        case NITROGEN_PURGE: {
            ...
            if (currentO2 <= 7.0f && currentO2 > 0) {
                Serial.println("Nitrogen purge complete (O2 <= 7%).");
        ...
        case PRIMARY_UV: {
            ...
            // Логика мерцания
            bool uv1_on = false;
            if (currentProcess.primary_uv_type1_enabled && currentProcess.primary_uv_type1_flickers > 0) {
                float period_ms = 1000.0f / currentProcess.primary_uv_type1_flickers;
                uv1_on = (fmod(elapsed, period_ms) < period_ms / 2.0f); // Скважность 50%
            }
            digitalWrite(UV_380_PIN, uv1_on ? HIGH : LOW);

            bool uv2_on = false;
            if (currentProcess.primary_uv_type2_enabled && currentProcess.primary_uv_type2_flickers > 0) {
                float period_ms = 1000.0f / currentProcess.primary_uv_type2_flickers;
                uv2_on = (fmod(elapsed, period_ms) < period_ms / 2.0f);
            }
            digitalWrite(UV_430_PIN, uv2_on ? HIGH : LOW);
            break;
        }
