...
const int DRIVER1_DIN_PIN = 27; 
const int DRIVER1_CIN_PIN = 26;
const int DRIVER2_DIN_PIN = 33; 
const int DRIVER2_CIN_PIN = 25; 
...
const int HEATER_PIN = 32;      // MOSFET 1, 
const int UV_380_PIN = 18;      // MOSFET 2,
const int UV_430_PIN = 5;       // MOSFET 3,
...
bool cooling_system_warning = false;
...
uint32_t flicker_on_ms = 0;                 // Рассчитанная длительность фазы "ВКЛ" в мс
uint32_t flicker_off_ms = 0;                // Рассчитанная длительность фазы "ВЫКЛ" в мс
...
void handleCommand(const String& cmd) {
    ...
    if (strcmp(command, "START_PROCESS") == 0) {
        ...
        driver2_g = 255; // Канал 'G' на драйвере 2
        applyDriverState();
        ...
        driver1_g = 255; // Канал 'G' на драйвере 1 (для охлаждения)
        applyDriverState();
        ...
        // Проверяем, что мы действительно на паузе и в нужном состоянии
        if (is_process_paused && currentState == NITROGEN_PURGE) {
            Serial.println("User chose to skip nitrogen. Resuming process.");
            is_process_paused = false; // Снимаем с паузы
            driver2_g = 0; // Выключаем клапан азота
            applyDriverState();
            SerialPort.println("WARN:NITROGEN_SKIPPED"); // Сообщаем об этом УП
            enterState(PRIMARY_UV); // Принудительно переходим на следующий этап
        }
    }
}
...
void enterState(ProcessState newState) {
    ...
    switch (currentState) {
        ...
        case HEATING_HOLDING: {
            // Serial.println("!!! DEBUG: Forcing FATAL HEATER FAILURE for test !!!");
            // all_off();
            // SerialPort.println("FATAL_ERROR:HEATER_FAILURE");
            // enterState(IDLE); // Сразу переходим в IDLE, чтобы остановить машину состояний
            // return; 
        ...
        case PRIMARY_UV: {
            ...
            // 2. Рассчитываем новые интервалы ON/OFF
            flicker_on_ms = 0;
            flicker_off_ms = 0;
            float on_time_sec = currentProcess.primary_uv_flicker_on_sec;

            // Режим мерцания активен, если время > 0 и < 1.0 секунды
        ...
        case POST_COOLING: {
            // Serial.println("!!! DEBUG: Forcing cooling_system_warning = true for test !!!");
            ...
            // Проверяем, нужно ли включать продувку сжатым воздухом
            if (currentProcess.chamber_cooling_enabled && currentProcess.post_cooling_air_purge_sec > 0) {
                ...
                driver1_g = 255; // Открываем клапан сжатого воздуха
            }
...
void state_machine_loop() {
    // Если процесс на паузе, мы НЕ выполняем НИКАКОЙ логики, включая отправку телеметрии.
    if (is_process_paused) {
        return;
    }
    ...
    switch (currentState) {
        ...
        case HEATING_HOLDING: {
            ...
            switch (heatingState) {
                ...
                case HS_COOLING:
                    ...
                    if (millis() - error_check_timer_start > 100000UL) { // Таймаут 100 
                        if (value_at_error_check_start - currentTemp < 0.5f) { // Если температура упала меньше чем на 0.5 градуса
                            Serial.println("!!! WARNING: Cooling system seems unresponsive. Skipping stage. !!!");
                            
                            // 1. Взводим флаг для финального отчета
                            cooling_system_warning = true;
                            ...
                            // 3. Сбрасываем подсостояние и пропускаем этап
                            heatingState = HS_IDLE;
                            SerialPort.println("WARN:COOLING_SKIPPED");
                            enterState(NITROGEN_PURGE);
                        } else {
                            // Если прогресс есть, сбрасываем таймер и обновляем точку отсчета
                            error_check_timer_start = millis();
                            value_at_error_check_start = currentTemp;
                        }
                    }
        ...
        case NITROGEN_PURGE: {
            ...
            if (millis() - error_check_timer_start > 100000) { // Таймаут 100 секунд
                if (value_at_error_check_start - currentO2 < 1.0f) {
                    ...
                    is_process_paused = true; // СТАВИМ НА ПАУЗУ
                } else {
                    error_check_timer_start = millis();
        ...
        case PRIMARY_UV: {
            ...
            // 2. НОВАЯ ЛОГИКА РАБОТЫ (ВКЛ/ВЫКЛ)
            float on_time_sec = currentProcess.primary_uv_flicker_on_sec;

            if (on_time_sec <= 0.0f) {
                // Режим "постоянно выключено", ничего не делаем.
                // Диоды уже выключены при входе в состояние.
            } else if (on_time_sec >= 1.0f) {
                // --- РЕЖИМ ПОСТОЯННОГО СВЕЧЕНИЯ ---
                if (!primary_uv_led_state) { // Включаем один раз, если еще не включены
        ...
        case POST_COOLING:
            // Проверяем, нужно ли выключить клапан продувки
            if (driver1_g > 0) { // Если клапан сейчас открыт
                if (millis() - state_timer_start >= (unsigned long)currentProcess.post_cooling_air_purge_sec * 1000) {
                    ...
                    driver1_g = 0; // Закрываем клапан
                    applyDriverState();
                }
            }

            // Основной таймер на 60 секунд для общего охлаждения
